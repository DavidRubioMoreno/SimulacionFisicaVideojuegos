...
void WindGenerator::updateParticles(double t)
{
	if (active) {
		Vector3 finalForce;
		Vector3 areaLimit1(centre.x + volume.x * 0.5, centre.y + volume.y * 0.5, centre.z + volume.z * 0.5);
		Vector3 areaLimit2(centre.x - volume.x * 0.5, centre.y - volume.y * 0.5, centre.z - volume.z * 0.5);
